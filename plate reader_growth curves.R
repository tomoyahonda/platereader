...
write.csv(df, file = "PlateReader_Growth_MG & RP_2.csv", na="")
...
#### calculate iGR #### 
Nu <- ncol(df) # define the original number of columns here before creating new columns as below
df[,ncol(df)+1:(ncol(df)+ncol(df))]<-NA # create empty columns
for (i in seq(4, Nu)) {
  for (j in seq(1, NROW(df[,i]))) {
    df[j,i+Nu-3] <- (log(df[j+1,i])-log(df[j,i]))/(df$Time[j+1]-df$Time[j])
  }
}
...
par(new=FALSE)
for (i in seq(4, length(df[,4:7])+3)) { #for glycerol
  plot(df[,i], df[,i+Nu-3], ylim=c(0,1.2), xlim=c(0.01,0.5), xaxs = "i", yaxs = "i" , log = "x",
       ylab = "iGR [hr-1]", xlab = "OD600", cex.lab=1.5, cex.axis=1.5,
       col="black", cex=2, pch=1, main = colnames(df[i])) 
  par(new=FALSE)
}

for (i in seq(8, length(df[,8:11])+7)) { #for 7AA
  plot(df[,i], df[,i+Nu-3], ylim=c(0,1), xlim=c(0.01,0.5), xaxs = "i", yaxs = "i" , log = "x",
       ylab = "iGR [hr-1]", xlab = "OD600", cex.lab=1.5, cex.axis=1.5,
       col="black", cex=2, pch=1, main = colnames(df[i])) 
  par(new=FALSE)
}
...
for (i in seq(16, length(df[,16:19])+15)) { #for glpK22
  plot(df[,i], df[,i+Nu-3], ylim=c(0,0.8), xlim=c(0.01,0.5), xaxs = "i", yaxs = "i" , log = "x",
       ylab = "iGR [hr-1]", xlab = "OD600", cex.lab=1.5, cex.axis=1.5,
       col="black", cex=2, pch=1, main = colnames(df[i])) 
  par(new=FALSE)
}

for (i in seq(20, length(df[,20:23])+19)) { #for glpK22
  plot(df[,i], df[,i+Nu-3], ylim=c(0,0.5), xlim=c(0.01,0.5), xaxs = "i", yaxs = "i" , log = "x",
       ylab = "iGR [hr-1]", xlab = "OD600", cex.lab=1.5, cex.axis=1.5,
       col="black", cex=2, pch=1, main = colnames(df[i])) 
  par(new=FALSE)
}